 ...
 */
export async function getTrackPagePromo(): Promise<{ storeName: string | null; promo: string | null }> {
  try {
    const t = await db.tenant.findFirst({
      where: { isActive: true },
      orderBy: { createdAt: "asc" },
      select: { name: true, trackPromo: true },
    });
    return { storeName: t?.name ?? null, promo: t?.trackPromo ?? null };
  } catch {
    return { storeName: null, promo: null };
  }
}
