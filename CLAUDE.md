...
- ✅ **Diskon Jasa Servis** — `ServiceTicket.discount` (Int rupiah); `total = laborCost + partsCost - discount`. `recompute` meng-clamp diskon ≤ (jasa+sparepart) & simpan nilai ter-clamp (total tak pernah negatif walau item dikurangi setelah diskon diisi). `updateDiscount` service + `updateServiceDiscountAction`. UI: input diskon di kartu Biaya (`ticket-detail.tsx`) + baris "Potongan diskon" + label "Lebih bayar" bila overpayment. Nota (visual+WA `buildServiceNotaText`) menampilkan baris Diskon. Basis-kas finance tetap benar (bayar ≤ total terdiskon; saat lunas: revenue=total terdiskon, COGS=penuh). Migrasi `20260825000000_add_service_discount`. Terverifikasi SSR (SV: Jasa 130rb − Diskon 50rb = Total 80rb di detail & nota).
