...
- ✅ **Simulasi Rakitan** (penawaran/quotation, di grup Layanan) — model `BuildSimulation`+`BuildSimulationItem` (nomor `SIM-xxxxx`, status DRAFT/SENT/APPROVED/IMPORTED/REJECTED). Simulasi TIDAK memotong stok (murni dokumen penawaran); komponen boleh dari inventory (autofill nama/modal/jual) ATAU bebas (ketik manual). Editor klien live (`src/app/(dashboard)/simulations/simulation-editor.tsx`): bujet & margin (Rp+%) dihitung real-time via `src/lib/build-sim-calc.ts` (`simTotals`, pure+unit-tested), sisa bujet berkurang saat komponen ditambah. Kirim penawaran via WA (`buildSimulationText`). **Impor ke Rakit PC** (`importToBuild`, transaksional): komponen inventory dgn stok cukup → dialokasikan (potong stok BUILD_OUT, `stockApplied=true`); komponen bebas / stok kurang → baris non-stok (`stockApplied=false`), tak potong. Guard impor-ganda via updateMany conditional dalam tx. **Perubahan skema Rakit PC**: `PcBuildItem.productId` kini NULLABLE + kolom `stockApplied Boolean` (removeComponent hanya restock bila `stockApplied && productId`). `src/server/build-sim/` (izin `pcbuild.manage`). Migrasi `20260824000000_add_build_simulations`. Terverifikasi: SSR list+detail dgn data nyata (Total Jual/modal/margin/sisa bujet akurat), typecheck, 6 unit test kalkulasi.
- ✅ **Nota Rakit PC** — `/pc-build/[id]/nota` (tombol "Cetak Nota" di detail rakitan). Nota berisi rincian komponen **tanpa harga per item**, hanya **TOTAL RAKITAN** (komponen + jasa rakit = `build.total`). Kirim ke pelanggan via WA gambar (`shareNodeAsImage`) atau teks (`buildPcBuildNotaText` di `src/lib/whatsapp.ts`), plus cetak. Pola sama dgn Nota Servis; `getBuildForNota` (pcbuild service) ambil no. HP pelanggan. Terverifikasi SSR (RKT-00003: 3 komponen tanpa harga, hanya total Rp 9.447.000).
- ✅ **Diskon Jasa Servis** — `ServiceTicket.discount` (Int rupiah); `total = max(0, laborCost + partsCost - discount)`. Diskon dibatasi ≤ (jasa+sparepart) saat DIISI (`updateDiscount`); `recompute` hanya floor total ke 0 TANPA menulis ulang discount (kalau ditulis ulang, diskon hilang bila item sempat dikurangi lalu ditambah lagi). `updateServiceDiscountAction`. UI: input diskon di kartu Biaya (`ticket-detail.tsx`) + baris "Potongan diskon" + label "Lebih bayar" bila overpayment. Nota (visual+WA `buildServiceNotaText`) menampilkan baris Diskon. Basis-kas finance benar: `serviceCogs` pakai rasio bayar/total-terdiskon (capai 100% saat lunas); `servicePartsRevenue` pakai rasio bayar/**gross** (sebelum diskon) agar tak melebihi pendapatan tertagih. Migrasi `20260825000000_add_service_discount`. Terverifikasi SSR (SV: Jasa 130rb − Diskon 50rb = Total 80rb di detail & nota).
- ✅ **Diskon Rakit PC** — `PcBuild.discount` (Int rupiah); `total = max(0, buildFee + componentsCost - discount)`. Pola sama dgn diskon servis: clamp ≤ (komponen+jasa) saat DIISI (`updateDiscount` pcbuild), `recompute` cuma floor total tanpa menulis ulang discount. `updateBuildDiscountAction`. UI: input diskon + baris "Potongan diskon" + "Lebih bayar" di kartu Biaya (`build-detail.tsx`). Nota Rakit PC menampilkan baris Subtotal + Diskon + Total bila ada diskon (tanpa diskon: hanya Total); harga per komponen tetap tersembunyi. Finance: `buildPartsRevenue` pakai rasio `total/gross` per rakitan agar tak melebihi pendapatan; `buildCogs` tetap penuh (akrual saat DONE/DELIVERED). Migrasi `20260826000000_add_build_discount`. Terverifikasi SSR (RKT-00003: gross 9.447.000 − diskon 447.000 = Total 9.000.000 di detail & nota).
...
- ✅ **Klaim RMA** (garansi ke distributor) — model `RmaClaim` (nomor `RMA-xxxxx`, produk/SN/keluhan, distributor+resi, sentAt/receivedAt, status SENT/RETURNED/REJECTED, resolution REPAIRED/REPLACED/REFUNDED + replacementSn, `warrantyUnitId` tautan ke garansi pelanggan) + `RmaPhoto` (pola ServicePhoto); `src/server/rma/` (izin inventory.manage); UI `/rma` (list+KPI+cari), `/rma/new` (autofill katalog/supplier/unit garansi, prefill `?wu=` dari tombol RMA di /warranty), `/rma/[id]` (terima kembali/tolak/buka lagi + catatan + foto + tautan garansi pelanggan + tombol "Beri tahu via WhatsApp"). **Tiket pelanggan**: `RmaClaim.customerName/customerPhone` (autofill dari unit garansi); pelanggan lacak status di `/lacak` (toggle Servis/Klaim Garansi, `trackRmaAction` di `src/server/public/track.ts`, label ramah via `rmaPublicStatusLabel`, verifikasi no.klaim+HP). Pengingat otomatis di Tanya Arta: klaim SENT >30 hari. Nav grup Inventaris. Terverifikasi end-to-end.
- ✅ **Dashboard Admin Platform** (operator SaaS, LINTAS-tenant) — `User.isSuperAdmin` (di-bootstrap dari env `SUPER_ADMIN_EMAILS` saat login; helper `src/lib/auth/super-admin.ts`; guard `requireSuperAdmin`/`requireSuperAdminAction`). `src/server/platform/` (stats, kelola lisensi/status/aktivasi semua toko, grant/cabut super-admin, CRUD kode promo). UI `src/app/admin/` (bukan grup dashboard; layout+guard, header navy: Ringkasan, Toko & Lisensi, Kode Promo, Akses Admin). **Kode promo/aktivasi**: model `PromoCode`+`PromoRedemption` (kode → paket lisensi + durasi hari + kuota); toko menukar di **Pengaturan** (`redeemPromoCode` transaksional, anti-tukar-ganda). **Pengaturan → Lisensi kini VIEW-ONLY + tukar kode** (editing lisensi pindah ke admin). Tautan "Dashboard Admin" di sidebar hanya untuk super-admin. Terverifikasi end-to-end (buat kode → tukar → edit lisensi → grant akses → guard menolak non-admin).
- ✅ **Panduan Penggunaan & Quick Start** — Manual lengkap 23 bagian (langkah + contoh per fitur) di `/about#panduan` (`src/app/about/manual.tsx`, accordion `<details>` tanpa JS); tautan "Panduan" di footer sidebar. **Quick Start** (`src/components/onboarding/quick-start.tsx`): dialog 5 langkah setup tampil SEKALI saat pertama buka dashboard (localStorage `artapos-quickstart-done`, pola useSyncExternalStore anti flash/lint), tiap langkah link ke menunya + tombol ke panduan lengkap. Terverifikasi (tampil→dismiss→tak muncul lagi setelah reload).
- ✅ **Halaman publik info/legal** — `/about`, `/disclaimer`, `/privacy`, `/terms` (semua rute publik via proxy OPEN_ROUTES; shell bersama `src/components/legal/legal-shell.tsx` = LegalShell/LegalSection/LegalBullets, footer auto-link ke-4). About: profil+fitur+kredit. Disclaimer: 9 section + as-is. Privacy: 10 section. Terms: 13 section (hukum Indonesia, ref Disclaimer). Konstanta brand+kredit di `src/lib/brand.ts` (Max Computer maxcomputer.id, Firman Ahmad @boysnocry firmanahmad.id). Tautan "Tentang · Disclaimer" di footer sidebar app & layout auth; halaman saling tertaut.
