...
- ✅ **Diskon Rakit PC** — `PcBuild.discount` (Int rupiah); `total = max(0, buildFee + componentsCost - discount)`. Pola sama dgn diskon servis: clamp ≤ (komponen+jasa) saat DIISI (`updateDiscount` pcbuild), `recompute` cuma floor total tanpa menulis ulang discount. `updateBuildDiscountAction`. UI: input diskon + baris "Potongan diskon" + "Lebih bayar" di kartu Biaya (`build-detail.tsx`). Nota Rakit PC menampilkan `total` yang sudah terdiskon (tetap "hanya total"). Finance: `buildPartsRevenue` pakai rasio `total/gross` per rakitan agar tak melebihi pendapatan; `buildCogs` tetap penuh (akrual saat DONE/DELIVERED). Migrasi `20260826000000_add_build_discount`. Terverifikasi SSR (RKT-00003: gross 9.447.000 − diskon 447.000 = Total 9.000.000 di detail & nota).
