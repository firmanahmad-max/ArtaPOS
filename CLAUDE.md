...
- ✅ **Klaim RMA** (garansi ke distributor) — model `RmaClaim` (nomor `RMA-xxxxx`, produk/SN/keluhan, distributor+resi, sentAt/receivedAt, status SENT/RETURNED/REJECTED, resolution REPAIRED/REPLACED/REFUNDED + replacementSn) + `RmaPhoto` (pola ServicePhoto); `src/server/rma/` (izin inventory.manage); UI `/rma` (list+KPI+cari), `/rma/new` (autofill katalog/supplier), `/rma/[id]` (terima kembali/tolak/buka lagi + catatan + foto). Nav grup Inventaris. Terverifikasi end-to-end.
